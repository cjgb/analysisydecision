...
Es una programación sencilla; seguramente haya una función específica en `scikit-learn` para agrupar variables en base a la frecuencia, pero no la he encontrado y he tardado más en buscarla que en hacerla.

Como es habitual, os pongo un ejemplo de uso para que podáis ver cómo funciona:
...
personas = 1000
grupo = pd.DataFrame(np.random.poisson(15, personas), columns=['grupo'])
valor = pd.DataFrame(np.random.uniform(100, 10000, personas), columns=['valor'])
df = pd.concat([grupo, valor], axis=1)

print(df['grupo'].value_counts().head())
```

Vemos que `grupo` crea muchos valores; vamos a agrupar la variable del *data frame* de forma que los 10 más frecuentes conserven su valor y los demás sean un "resto":

```python
agrupa_frecuencia('grupo', 'grupo_nuevo', df, 10, 99)
print(df['grupo_nuevo'].value_counts())
```

Parece que funciona; si mejoráis, actualizáis o encontráis pegas… Saludos.