...
Para `analizar` la `sensibilidad` y la `especificidad` de nuestra tenemos que establecer unos `puntos de corte` y elaborar para cada `punto de corte` una `tabla 2x2` de este `modo`:

![](/images/2014/02/prueba_diagnostica_roc_sensibilidad_especificidad_1.jpg)

Vamos a realizar una `tabla 2x2` para todos los `puntos de corte` de la `medida` del `VCM` que van desde 60 hasta 95, almacenamos en un `objeto` los `valores` de la `sensiblidad` y la `especificidad` para posteriormente poder realizar la `representación gráfica`:
