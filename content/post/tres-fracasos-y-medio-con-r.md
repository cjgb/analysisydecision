...
El `segundo fracaso` lo he `cosechado` con una `versión` más `sofisticada` de `read.table` (es decir, `read.table` con `guirnaldas` y `abalorios` para, como `dicen` los que `hacen llorar` a `Cervantes`, `eficientar` la `lectura` de `datos`:`bigdata <- read.table( "test_data.txt", header = T, sep = "\t", comment.char = "", colClasses = c( "numeric", "factor", "numeric", "factor", "numeric", "factor", "factor", "factor", "factor", "factor", "numeric", "numeric", "numeric", "factor", "factor" ) )`)
