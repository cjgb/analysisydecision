...
Hoy, mientras Raúl departía en el cuarto de al lado —él os dirá por qué y para qué— y hacía tiempo para saludarlo, me he entretenido fracasando tres veces y media en lo que abajo desarrollo. No pensaba publicarlo hasta que, reflexionando, he considerado que, a veces, es más útil una buena pregunta que algunas certezas. Inconcluso —o fracasado— no tiene por qué significar inútil o imperfecto.

Veamos en qué han consistido mis fraccionarios fracasos.

He intentado algo "imposible": leer un fichero de texto de dos gigas en un ordenador con unas pocas gigas de `RAM`. Como no tenía un fichero de tal magnitud a mano, me lo he fabricado a partir de otro de cuatro "meguillas":

```r
# Fabricando un fichero grande
asdf <- read.table("fichero_de_4_megas.txt", header = TRUE, sep = "\t")
write.table(asdf, file = "test_data.txt", row.names = FALSE, sep = "\t")

for(i in 1:500) {
  write.table(asdf, file = "test_data.txt", row.names = FALSE, 
...
```r
bigdata <- read.table("test_data.txt", header = TRUE, sep = "\t")
```
...
Eliminar el carácter de comentario y predefinir el tipo de columnas sólo ha servido para acelerar el momento en el que `malloc` se ha pegado un cabezazo contra el techo de la `RAM`. Fracaso dos.

A la tercera he tenido un **medio éxito** —¿de qué estaría hablando Raúl tanto rato?— utilizando [un paquete que publicó en CRAN un personaje de mala calaña](http://cran.r-project.org/web/packages/colbycol/index.html) al que no hay que creer nada de cuanto dice o escribe: `colbycol`.

```r
library(colbycol)
system.time(bigdata <- cbc.read.table("test_data.txt", header = TRUE, sep = "\t"))
```
...
El **cuarto ensayo** ha sido un [remedo de algo que leí](http://www.cerebralmastication.com/?p=416). Tenía ganas de probar [`sqldf`](http://cran.r-project.org/web/packages/sqldf/index.html) (y también `SQLite`, claro). ¿Que qué es [`SQLite`](http://www.sqlite.org/)? Pues es [una base de datos que algunos lleváis en el bolsillo](http://forums.macrumors.com/showthread.php?t=453043) (en el `iPhone`, por ejemplo). De hecho, la llevaba un tipo que alegaba que, en bases de datos, `Oracle` y nada más…
...
system.time(bigdata <- sqldf("select * from tmp", dbname = tempfile(), 
