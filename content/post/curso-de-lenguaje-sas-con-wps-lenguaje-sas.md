...
Por defecto la `engine` es `WPD`, es decir, crear ficheros WPS. Para trabajar con ficheros `SAS7BDAT` tenemos que usar esa `engine`, esto nos facilitará a la perfección la migración de nuestros procesos SAS a WPS. A lo largo de las sucesivas entradas veremos cómo asignar librerías y cómo trabajar con ellas.