...
Podéis ver que la función de `worldfootballR` que se está usando es `fb_match_results`; creo que es la más rápida y cómoda, pero como estos datos están pensados para representar datos en una web, es necesario cocinar un poco y preparar un conjunto de datos por `Equipo-Jornada-Resultado`.
...
partidos3 <- partidos3 %>% left_join(liga, by = "Equipo")
```

Ya están todos los elementos necesarios para realizar el gráfico con el que comienza esta entrada.
