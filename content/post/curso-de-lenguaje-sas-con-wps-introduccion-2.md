...
Sí, es posible tener una arquitectura cliente-servidor y es posible montarlo en un *mainframe*. Aunque el curso va muy orientado a trabajar en un PC, si es necesario, montamos una máquina virtual y también trabajamos cliente-servidor. Los módulos disponibles en `WPS` son:

- `WPS Core`: es un intérprete de `SAS` y soporta lenguaje `BASE`, `macros`, `ODS` y `formatos`.
...
Estas son las posibilidades que nos ofrece `WPS`. De nuevo, insisto en que en estas líneas nos vamos a centrar en trabajar con lenguaje `BASE`; si detecto un mayor interés en otros módulos, trabajaremos sobre ellos.
