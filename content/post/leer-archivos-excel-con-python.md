...
Lo más sencillo para importarnos in `Python` un `Excel` y crearnos un `data.frame` de `Pandas` es:
...
Hay alguna librería que lo hace de forma más elegante pero la importación de rangos de `Excel` con `xlrd`, una vez te familiarizas, me parece bastante sencilla. Espero que sea de utilidad. Saludos.
