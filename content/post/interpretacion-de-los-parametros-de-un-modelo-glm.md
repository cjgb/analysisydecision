...
El modelo siempre fija un nivel base del factor, un nivel que promedia nuestros datos, y el resto de niveles corrigen el promedio en base al coeficiente estimado. Imaginemos que modelizamos el número de abandonos en 3 carreras de coches: cuando la carrera se disputa en un circuito A el número de abandonos es 5, sin embargo en el circuito B son 10 y en el circuito C son 15. Si fijamos como nivel base de nuestro factor circuito el B, tendríamos un modelo de este modo: $\text{abandonos} = 10 + 0.5 \times \text{es circuito A} + 1 \times \text{es circuito B} + 1.5 \times \text{es circuito C} + \text{Error}$. El nivel base promedia nuestro modelo por lo que va multiplicado por 1 y el resto de niveles se corrigen por su multiplicador. Esta es la base de la modelización multivariante en el sector asegurador. Veamos en un ejemplo cómo se articulan los parámetros de estos modelos. Simulamos unos datos con la probabilidad de tener un siniestro por edad, zona y sexo:
