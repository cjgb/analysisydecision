...
"Dumificar" es crear variables *dummy*. Un verbo completamente inventado pero que todos los que os habéis enfrentado a la creación de una tabla de entrada para realizar modelos estadísticos vais a entender perfectamente en qué consiste. Dumificar es transformar una variable continua en $N$ variables dicotómicas. Lo entenderemos mejor con un ejemplo gráfico:

![dumificar.JPG](/images/2009/04/dumificar.JPG)
...
* DATASET ALEATORIO QUE HAY QUE AGRUPAR EN FUNCION DE LA VARIABLE IMPORTE;
data uno;
  do i = 1 to 334560;
    importe = ranuni(0) * 1000;
    if rand("uniform") < 0.04 then resp = 1;
    else resp = 0;
    
    * Simulación de cierta relación;
    if resp = 0 and 200 < importe < 400 then if rand("uniform") < 0.1 then resp = 1;
    if resp = 0 and 800 < importe < 900 then if rand("uniform") < 0.2 then resp = 1;
...
proc delete data=instruccion; run;
