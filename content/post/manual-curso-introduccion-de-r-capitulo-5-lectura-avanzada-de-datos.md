...
Para leer un fichero de texto emplearemos un fichero de ejemplo `GRADES.TXT`, que es un fichero de texto delimitado por espacios y sin cabeceras; tiene los siguientes campos:
...
Descárgatelo [aquí](/images/2008/03/grades.TXT) y guárdalo en `C:\WINDOWS\temp` para seguir el ejemplo. La función que se emplea en R para leer tablas desde ficheros de texto es `read.table`:
...
Vemos que la sintaxis de la función `read.table` es bastante sencilla. Recomiendo estudiar la documentación que tiene R al respecto de esta función. El argumento principal de esta función será `file = <ubicación y nombre del fichero>`; además, con `header` indicamos si es necesario leer cabeceras; por defecto `read.table` lo tiene a `FALSE`. Para introducir las cabeceras al fichero empleamos vectores:
...
Otra fuente de datos muy habitual puede ser Access. Para leer bases de datos necesitaremos tener instalado el paquete `RODBC`. En el capítulo anterior se indicó cómo descargar e instalar este módulo de R. Para este ejemplo partimos de una BBDD Access `bd1.mdb` almacenada en `C:/WINDOWS/temp` que contiene la siguiente tabla:
...
Podéis crear la BBDD en la ubicación `C:/WINDOWS/temp`, copiar y pegar el ejemplo para estudiar cómo funciona el código. Comencemos a analizar las sentencias en R:
