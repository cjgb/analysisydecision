...
En este punto, más que mostrar cómo leer datos de la OECD, casi pido ayuda para entender la API y la librería. Si ejecutamos:
...
No encuentro la tabla `SH.XPD.CHEX.GD.ZS` que contiene *Current health expenditure (% of GDP)*, el gasto en sanidad sobre % de PIB; tampoco he llegado a comprender bien la API para bajarme el XML o el JSON; a los 45 minutos de lucha me rendí. Así que opté por descargar el Excel, tratarlo manualmente y leerlo.
...
Qué grandes gráficos de bivariables se hacen con `ggplot`. Además, la sintaxis no puede ser más sencilla:
...
Qué gran colocación de las etiquetas en el gráfico bivariable; podríamos añadirle más países y ponerle las cosas más difíciles.
