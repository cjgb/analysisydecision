...
url: /evaluando-la-capacidad-predictiva-de-mi-modelo-tweedie/
---
...
[Una vez ejecutado el proceso que realizaba el modelo tweedie](/images/2020/12/modelos_tweedie.txt) disponíamos de un objeto `norauto` con las variable `prima_estimada` que era el resultado de la estimación de nuestro modelo tweedie sobre la prima pura de la cartera de automóviles con la que estamos trabajando. ¿Esa estimación es buena? ¿Cómo sé si es buena? Para medir la capacidad predictiva propongo **contrastar la estimación frente al azar** , lo que se llama «ganancia». 
...
resumen_aleatorio <- norauto %>% group_by(tramos=tramos_aleatorios) %>%
  summarise(porcen_aleatorio = sum(ClaimAmount)/sum(norauto$ClaimAmount))
...
resumen_estimacion <- norauto %>% group_by(tramos=tramos_estimados) %>%
  summarise(porcen_estimado = sum(ClaimAmount)/sum(norauto$ClaimAmount))
