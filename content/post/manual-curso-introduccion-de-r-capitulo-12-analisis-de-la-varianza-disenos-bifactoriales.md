...
Con ello el modelo matemático sería: $Y_{ij} = \text{media} + \text{media}_i + \text{media}_j + \text{media}_{ij} + e_{ij}$. Con las hipótesis de suma de los coeficientes asociados igual a 0 y distribución normal del error. Trabajemos con un ejemplo para conocer las sentencias de R:
...
Nuestro experimento tiene como variable respuesta la capacidad de penetración y en él intervienen 2 factores: la taladradora empleada y la broca. Así pues, tenemos un modelo bifactorial de efectos fijos. El modelo matemático es: $y_{ij} = \text{media} + \text{media}_i + \text{media}_j + \text{media}_{ij} + \text{error}_{ij}$, con $i \in \{1, 2, 3, 4\}$ y $j \in \{1, 2, 3\}$. Donde $y_{ij}$ es el valor de la variable respuesta para la taladradora $i$ con la broca $j$, $\text{media}$ es la media global, $\text{media}_i$ es la media para la taladradora $i$-ésima, $\text{media}_j$ es la media para la taladradora $j$-ésima, $\text{media}_{ij}$ es la media de la interacción entre los factores y el $\text{error}_{ij}$ es el error. Las condiciones son que la suma de las medias sea 0 y que el error se distribuye normalmente con media 0. Introduzcamos los datos en R:
