...
tags:
  - cruce de tablas sas
  - hash
title: Trucos SAS. Por qué hay que usar objetos hash
...
Tenemos que cruzar el *dataset* `grande` con el `pequenio`, 20 millones frente a unos miles. En mi larga (muy larga) experiencia trabajando con SAS, casi todo el mundo (menos Sonia) con el que he coincidido haría:

```sas
* OPCION 1: ORDENACION Y MERGE;
proc sort data=grande; by idcliente;
proc sort data=pequenio; by idcliente; 
run;
...
proc sort data=pequenio out=selec nodupkey; by idcliente; run;
