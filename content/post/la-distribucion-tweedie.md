...
Este tipo de distribución necesita 3 parámetros: `p`, que nos indica el tipo de distribución y en R se denomina `power`; `mu`, que es la media; y `phi`, que es la desviación típica. En R disponemos del paquete tweedie para trabajar con este tipo de distribuciones; la función `rtweedie` es la que genera números aleatorios según una Tweedie con parámetros `p`, `mu` y `phi`:
