...
%macro determina_tamanio(p0, p1, prob);
    ...
    data _null_;
        do n = 1 to 5000;
            ...
            /* LÍNEA CENTRAL */
            lc = &p0. * n;
            /* LÍMITE INFERIOR (SIN NEGATIVOS) */
            lic = max(&p0. * n - 3 * sqrt(&p0. * (1 - &p0.) * n), 0);
            
            /* DIFERENCIA CON RESPECTO AL LÍMITE SUPERIOR */
            z = (lsc - (n * &p1.)) / sqrt(n * &p1. * (1 - &p1.));
            p_calc = probnorm(z);
            
            if round(p_calc, 0.01) = &prob. then put n= p_calc=;
        end;
    run;
%mend;
...
Definimos el límite superior, la línea central y el límite inferior para la fracción `p0`. Sólo necesitamos el límite superior para el ejemplo que estamos tratando, pero ahí lo tenéis. Entonces, determinamos la diferencia con la nueva fracción de no conformes `p1` y estandarizamos. Calculamos la probabilidad que deja la normal del dato estandarizado y, si es parecida a la que nos pide el proceso, la mostramos en la ventana *log*. 

En el ejemplo que he puesto, determina el tamaño muestral mínimo para detectar un cambio en la fracción de no conformes de 0.1 a 0.2 con una probabilidad del 25%. Espero que a alguien le sea de utilidad. Saludos.