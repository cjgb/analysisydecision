...
df <- df %>%
  filter(ambito == 'nacional' & nombre_sexo == 'todos' & cod_gedad == 'all') %>%
...
ggplot(data = df, aes(x = fecha_defuncion, y = defunciones_observadas, group = ola, fill = ola, color = ola)) +
  geom_line() +
  geom_line(aes(x = fecha_defuncion, y = defunciones_esperadas), color = "grey")

df %>%
  group_by(ola) %>%
  summarise(exceso = sum(exceso))
```
