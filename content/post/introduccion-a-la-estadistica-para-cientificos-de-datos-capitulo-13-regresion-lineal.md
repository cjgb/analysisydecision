...
No se distribuye normalmente y para corroborarlo se disponen de gráficos QQ que compara los cuantiles de la distribucion normal frente a los cuantiles de la distribución de una variable.
...
Diagnóstico del modelo. Además del R^2 es necesario validar y diagnóstica si se cumplen todas las hipótesis del modelo lineal:
...
```r
estimacion.modelo.1 <- predict(object=modelo.1, data=cost_livingRent.Index)
...
[![](/images/2023/02/wp_editor_md_ba14629e0e5a0d63e5528780dcc3c253.jpg)](/images/2023/02/wp_editor_md_ba14629e0e5a0d63e5528780dcc3c253.jpg)
...
El primer gráfico recoge los residuos frente al ajuste, en estimaciones superiores a un índice de 80, -20% con respecto a NYC, hay un patrón que el modelo lineal no recoge. Lo corrobora el siguiente gráfico que estudia la normalidad de los residuos, falla en ambos extremos de la estimación pero más en estimaciones superiores. El gráfido de **scale – location** estudia la homocedasticidad, los residuos estudentizados deberían situarse sobre una línea central para asumir igualdad de varianza y esto no sucede. El último gráfico permite estudiar si hay residuos que estén influyendo sobre los resultados del modelo, se identifican la observación 12 y 14:
...
Y = \\beta_0 + \\beta_1_Rent.Index = 1. \<=15 + \\beta_2_Rent.Index = 2. 16-30 + \\beta_3_Rent.Index = 3. 31-45 + \\beta_4_Rent.Index = 4. 46-60 + \\beta_5\*Rent.Index = 5. mas de 60
...
En este ensayo se van a ilustrar los tres métodos basando la capacidad predictiva de cada método en el **critero de información de Akaike** conocido como AIC. Al igual que el R^2 el AIC es una medida de lo correcto que es el ajuste pero está ponderado por el número de parámetros del modelo, sin entrar en aspectos teóricos cuanto menor es el AIC mejor es el modelo y si una variable no disminuye el AIC, no lo mejora sustancialmente, esta variable será prescindible.
