...
![Imagen de previsualización de YouTube](https://img.youtube.com/vi/Fiis4AQbXug/0.jpg)

«Pagar por un `alud` en `Sevilla` no tiene mucho `sentido`». Este `comentario` va en la `línea` de la `personalización` de las `tarifas` como sucede con el `producto` [`YCAR de Mapfre`](%60http://www.mapfre.com/seguros/es/particulares/soluciones/seguros-coches-jovenes-ycar.shtml%60) o la [`personalización` del `seguro` de `Clickseguros`](%60http://www.clickseguros.es/seguros_de_coche/noticias_seguros_de_coche/Paginas/seguros-a-medida.aspx%60), esta última es muy `peligrosa` hacerla con un `producto directo` sin la `asesoría` de un `experto`. Estos `productos personalizados` requieren un mayor «`esfuerzo actuarial y técnico`» pero parece que el `futuro` pasa por ellos. La `tarifa a medida`.
