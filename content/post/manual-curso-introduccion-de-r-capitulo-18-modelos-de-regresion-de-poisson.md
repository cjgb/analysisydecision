...
Empleamos `GLM` para la creación del modelo, la variable dependiente evidentemente será el número de reclamaciones, como `offset` o variable de exposición tenemos el logaritmo del tamaño de la población, las variables independientes serán el `tamanio_coche` y el `grupo_edad`. Con `family` indicamos que se realice el modelo de `poisson()`. Resumamos el modelo:
...
El primer paso es validar la hipótesis de varianza = media que define una distribución de Poisson. Para ello hacemos `desviación/grados de libertad` en nuestro caso $2.8/4 = 1.4$. Parece que tenemos una ligera sobredispersión. Ya estamos en disposición de emplear nuestro modelo para realizar estimaciones:
