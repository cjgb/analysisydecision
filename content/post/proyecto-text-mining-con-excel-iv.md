...
Empezamos a tener datos tabulados en la hoja `Hoja1` como producto de la `Copy` de los datos de la hoja `search`. Ahora tenemos que diseñar un proceso que realice todos estos pasos de forma automática hasta un número de entradas determinado. Así dispondremos de nuestro tablón de datos. Pero esto queda para una posterior entrega.
