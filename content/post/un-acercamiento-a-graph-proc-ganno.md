...
El procedimiento SAS `GANNO` crea gráficos a partir de conjuntos de datos SAS `ANNOTATE`. Estos conjuntos de datos SAS `ANNOTATE` recogen las distintas formas o cuadros de texto que tú quieres representar. Estos conjuntos de datos han de tener unas variables que son las que recogen las formas a pintar; imprescindibles son:

```sas
length function $8 style $20 color $8 text $25;
...
```sas
goptions reset=global cback='red' colors=(black);
...
```sas
goptions reset=global cback='blue' colors=(black);
...
Tenemos cuatro cuadros de texto que contienen las coordenadas de los puntos indicados. Podemos combinarlo con la función `draw` y hacer un dibujo:
...
data dibujo;
    ...
    %move(50, 50);
    %circle(50, 50, 50);
    %rect(50 - sqrt((50**2) / 2), 50 - sqrt((50**2) / 2), 
          50 + sqrt((50**2) / 2), 50 + sqrt((50**2) / 2), 
          black, 1, 1);
run;
```
...
```sas
goptions reset=global cback='blue' colors=(black) device=gif xpixels=500 ypixels=500;
...
goptions reset=global cback='blue' colors=(black) device=gif gsfname=a xpixels=500 ypixels=500;
```
