...
Hemos creado el objeto `asignacion` que asigna a cada observación el `cluster` resultante del análisis no jerárquico. Este objeto lo unimos con `analisis.ok` y podemos analizar como se ha comportado el agrupamiento:
