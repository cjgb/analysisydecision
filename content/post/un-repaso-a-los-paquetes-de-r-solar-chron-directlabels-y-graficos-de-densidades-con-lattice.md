...
fechas <- as.Date(row.names(datos), "%Y-%m-%d")
datos$dia <- factor(weekdays(fechas))
datos$mes <- factor(as.yearmon(row.names(datos)))
datos$finde <- factor(ifelse(is.weekend(fechas), "FINDE", "RESTO"))
...
densityplot(~ lluvias | mes, data = datos, groups = finde,
            layout = c(3, 4), xlab = "Lluvias", bw = 2, plot.points = FALSE)
```
