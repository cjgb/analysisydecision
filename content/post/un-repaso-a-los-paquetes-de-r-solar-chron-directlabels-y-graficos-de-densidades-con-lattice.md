...
![lluvias-1.png](/images/2011/05/lluvias-1.thumbnail.png "lluvias-1.png")

Echamos de menos un `etiquetado` para el `gráfic`o de `densidades` que hemos creado con `densityplot` , para ello vamos a emplear `directlabels`:
...
Este gráfico es claramente mejorable, además incluye un error pero mi hijo acaba de entrar por la puerta llena de tierra con una espátula y… En fin, los `resultados` obtenidos son `interesantes`. Como `ejercicio` os dejo lo siguiente: `réplica` del `gráfic`o con `ggplot2` y `análisis` de la `varianza`.
