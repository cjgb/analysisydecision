...
1. Realizar una primera importación desde `SAS`, preferiblemente `SAS Base` no `Enterprise Guide` y comprobar que problemas plantea la importación.
1. Analizar, si fuera necesario, el tipo de problema (delimitación, cabeceras, configuración, fechas) que plantea la importación.
...
1. Emplear el asistente para realizar una importación de prueba, es necesario tener especial cuidado con la delimitación del fichero, recomiendo trabajar con tabuladores y ficheros `*.txt`
1. Analizar los problemas surgidos de la importación
1. Recuperar el código interno `SAS` de la importación pulsando `F4` en el editor, siempre aparece en la parte superior
1. Modificar el `paso data` que genera la importación: Nombres de variables y formatos de entrada
