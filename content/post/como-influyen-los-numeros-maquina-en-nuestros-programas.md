...
La función `valores_maquina` tiene un solo parámetro y realiza una operación que genera el valor `0.1`; con `ifelse`, si ese valor generado es menor que `0.1` nos pondrá `OK`, si no es menor nos pondrá `MAL`. En principio siempre debería poner el valor `MAL`, ya que `0.1 < 0.1` es falso. Entonces:
