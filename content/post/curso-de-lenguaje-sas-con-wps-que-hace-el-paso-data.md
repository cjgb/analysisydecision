...
Para crear `datasets` desde `ficheros de texto` emplearemos una `metodología` distinta en `WPS` que la utilizada en `SAS`. `WPS` no tiene `asistentes` por ello lo mejor que podemos hacer es importar el `fichero paso` a `paso` para comprobar si lo hacemos correctamente. Para ilustrar el ejemplo [podéis bajaros este archivo](%60/images/2008/03/grades.TXT%60) a vuestro equipo. Este fichero tiene las siguientes `variables`:
