...
![](/images/2022/02/actor_senil_pre-300x122.png)

Este caso es demasiado fácil ya que, con la información recibida, las posibles palabras dentro de la `RAE` se limitan a una:
...
![](/images/2022/02/actor_senil-258x300.png)

### Ejemplo 2

![](/images/2022/02/actor_senil_2_pre-300x122.png)
...
Al existir múltiples opciones (aunque una clara candidata), decido limitar la búsqueda a la base de datos del Quijote, que es **lo más parecido a lo que haría nuestro cerebro**, obteniendo solo una palabra posible:
...
En conclusión, y a la espera de verificar cómo influiría el posicionamiento, podemos decir que `ACTOR` y `SENIL` serían las mejores palabras para comenzar en el `Wordle`. `LICOR` y `ANTES` si hubiésemos nacido en el siglo XVII.
