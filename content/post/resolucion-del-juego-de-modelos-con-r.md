...
![juego_modelos1](/images/2016/05/juego_modelos1.png)
...
![juego_modelos2](/images/2016/05/juego_modelos2.png)
...
![juego_modelos3](/images/2016/05/juego_modelos3.png)
...
![juego_modelos3b](/images/2016/05/juego_modelos3b.png)

Observad que a los árboles les "cuesta una peonada" salir de la nube de puntos en sus estimaciones. Mis queridas redes neuronales siempre necesitan más trabajo, pero es muy interesante cómo salva la linealidad `nnet` con una sola capa oculta y 5 nodos:

```r
# Redes neuronales
library(nnet)

# Función para encontrar la mejor red neuronal en base al menor RSS
mejor.rss <- Inf
for (i in 1:50) {
  modelo.rn <- nnet(dep ~ indep, entrenamiento, size = 5, linout = T, trace = F, decay = 0.2)
...
plot(test)
points(test$indep, predict(modelo.3, test), col = "red")
```

![juego_modelos4](/images/2016/05/juego_modelos4.png)
...
![juego_modelos5](/images/2016/05/juego_modelos5.png)
...
![juego_modelos6](/images/2016/05/juego_modelos6.png)

Con sobreajuste el SVM puede ofrecer alguna estimación fuera de la nube de puntos; sin embargo, siendo menos estrictos con esos parámetros los SVM quedan siempre dentro de la nube de puntos y su estimación se parece mucho a la que pueda ofrecernos una red neuronal. El KNN tampoco va a sacar ningún punto del test de la nube de puntos:

```r
# KNN
library(kknn)

modelo.6 = kknn(dep ~ indep, entrenamiento, test, k = 4, kernel = "rectangular")
plot(test)
points(test$indep, predict(modelo.6, test), col = "red")
```

![juego_modelos7](/images/2016/05/juego_modelos7.png)

Otro de mis sobreajustadores preferidos y, por último, el clásico `loess` que siempre nos ha gustado mucho a los dinosaurios:

```r
# Loess
modelo.7 = loess(dep ~ indep, entrenamiento)
plot(test)
