...
depos_html <- readHTMLTable(pag)

# Creamos un data frame legible
aux1 <- data.frame(depos_html[6])
...
valor <- as.data.frame(as.numeric(levels(aux1$NULL.V2)[aux1$NULL.V2]))

# Creo el df final
depos <- data.frame(mes, interes = valor)
...
eur6m_html <- readHTMLTable(pag2)

aux1 <- data.frame(eur6m_html[6])
...
eur6m <- data.frame(mes, eur6m = valor)
eur6m$mes <- as.character(eur6m$mes)
...
# Unimos ambas series
datos <- merge(depos, eur6m, by = "mes", all.x = TRUE)
```
...
```r
# Damos un formato apropiado a las fechas
...
mes_num <- (as.numeric(format(aux2, format = "%Y")) * 100 
            + as.numeric(format(aux2, format = "%m")))

# Creamos el DF final
datos_final <- data.frame(mes = mes_num, interes = datos$interes, eur6m = datos$eur6m)

# Tenemos que realizar una ordenación
library(reshape)
datos_final <- sort_df(datos_final, vars = 'mes')
```
...
```r
plot(datos_final$interes, ylim = c(0, 6), type = "l", lwd = 2,
     xaxs = "r", yaxs = "r", xaxt = "n", yaxt = "n",
     ylab = "%", col = 14, xlab = "", panel.first = grid(lwd = 1),
     main = "Interés medio depósitos frente Euribor 6 m")

points(datos_final$eur6m, type = "l", col = 12, lwd = 2)

axis(1, 1:nrow(datos_final), datos_final$mes)
axis(2)
axis(4)
