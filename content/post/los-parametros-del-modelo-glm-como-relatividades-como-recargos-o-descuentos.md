...
glM.1 <- glm(nsin ~ edad_factor + offset(log(exposicion)), 
             data = filter(moto, exposicion > 0),
             family = poisson())
summary(glm.1)
```

Hemos creado un factor edad que va desde los 18 años hasta los 60; realizamos una regresión de Poisson para estimar el número de siniestros. Como al final lo que deseamos es crear una proporción de siniestros de la forma `nsin / exposicion` (frecuencia siniestral), lo que hacemos es poner el `nsin` como variable dependiente y la exposición como variable `offset`. La única variable regresora es la edad en formato `factor`. Con este modelo obtendremos un estimador para cada nivel del factor. Es un modelo aditivo de la forma $\log(Y) = \beta_0 + \text{Edad19} \cdot \beta_1 + \text{Edad20} \cdot \beta_2 + \dots + \log(\text{exp}) + \text{Error}$, pero si realizamos el exponencial de los parámetros obtenidos con el modelo tendremos $E[Y/\text{exp}] = \beta'_0 \cdot \text{relatividad\_Edad19} \cdot \dots$. Es decir, el valor esperado para la frecuencia siniestral es función de unos parámetros que recargan o descuentan esa frecuencia esperada. Esos $\beta'$ que son el resultado de $\exp(\beta)$ es lo que denominamos relatividades. Esto es muy utilizado para la realización de modelos de riesgo en el cálculo de tarifas.

## Obtención de las relatividades

Reiterando: el exponencial del parámetro obtenido con la formulación del modelo es lo que denominamos relatividad, y esa relatividad multiplicada por un término independiente nos daría como resultado la estimación de la proporción de siniestros, la estimación de la frecuencia siniestral para cada nivel del factor. También es relevante estudiar y comprender cómo R presenta esos parámetros; si hacemos el exponencial de los parámetros del modelo `glm.1` que hemos hecho tenemos:
...
```r
factor_reclasificado <- moto %>%
  group_by(edad_factor) %>%
  summarise(exp = sum(exposicion))

rela = exp(glm.1$coefficients)
# El intercept es la tasa del nivel base, para relatividades lo ponemos a 1
rela[1] <- 1 
...
```r
library(ggplot2)
ggplot(relatividades, aes(x = edad_factor)) +
  geom_col(aes(y = exp), fill = "yellow", alpha = 0.5) +
  geom_line(aes(y = rela * 5000), group = 1, color = "red") +
  scale_y_continuous(sec.axis = sec_axis(~ . / 5000, name = "Relatividad"), name = "Exposición") +
  theme_light()
```
