...
Podemos seguir mejorando el `heatmap` de juego con funciones como `stat_density_2d` que sirve para mejorar esas densidades `bivariada`. En este caso se aplica por duplicado, una para hacer `contornos` y otra para hacer establecer `las` «`zonas` `rojas`». Además podemos eliminar `leyendas`, mejorar `títulos`,…
...
ubicacionx <- coordenadas$x; ubicaciony<-coordenadas$y
