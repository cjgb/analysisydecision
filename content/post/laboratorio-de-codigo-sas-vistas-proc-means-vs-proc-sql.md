...
El código es más farragoso. ¿De verdad compensa in tiempo de ejecución la creación de vistas? Vamos a dar respuesta con la famosa macro que lanza ejecuciones y guarda tiempos. El método 1 es vista + means y el método 2 es `SQL` (he cambiado el orden):
...
Ejecutadlo. El método 1 (vista + means) es mucho más rápido in ejecución. Aunque con 2 millones de registros los tiempos no son destacables, si trabajamos con un volumen mayor, un 20% más de tiempo de ejecución puede ser mucho. **Hay que trabajar más con vistas**, incluso in `SAS`. Saludos.
