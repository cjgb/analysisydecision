...
Coinciden el `atributo brks` (los `cortes` de nuestra `clasificacion`) y los `cuantiles` de nuestro `vector`. Este paquete lo estoy empleando conjuntamente con `RcolorBrewer` para la realización de gráficos:
...
Es un objeto con los `colores` por `elemento` del `vector` pero además contiene los `colores` y los `atributos` necesarios para poder realizar el `grafico` como es la `tabla` y las posibles `etiquetas` del `grafico`. Si deseamos modificar las `etiquetas` o `leyendas` en los `graficos` yo recomiendo trabajar con el `atributo brks` del objeto `classInt`, en este caso `clases`:
