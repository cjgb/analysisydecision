...
En la primera ejecución, la que genera el dataset `tres` , empleamos `IF`, el tiempo real de la ejecución (en mi caso) es 5,58 s. para el `WHERE` como instrucción que genera el `dataset` `cuatro` es de 11,9 s. y si empleamos `WHERE` como opción de lectura de los `dataset` para crear `cinco` tenemos un tiempo de 14,41 s. Es evidente que el `IF` tiene un comportamiento más óptimo. Por otro lado si en tu trabajo es necesario _picar_ mucho código el `IF` puede ahorrarnos tiempo. Con estos mismos ejemplos, si deseamos los pares y que además tengan el valor de la variable `aleatorio1` > 0,5 no podremos emplear en el `MERGE` el `WHERE` porque no está en ambos `dataset`s:
