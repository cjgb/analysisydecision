...
La **inferencia paramétrica** puede recogerse en una vertiente o en otra según el parámetro a estimar; tenemos por un lado la estadística clásica (que es en la que nos vamos a centrar) y por otro lado la estadística bayesiana.
...
"La chi-cuadrado es una suma de normales al cuadrado"; más o menos se podría definir así, ya que si calculamos la distribución de una variable normal al cuadrado no podemos aplicar cambio de variable y, a partir de su función de distribución, llegamos a una función de densidad de una gamma con parámetros $1/2$ y $1/2$ que es una chi-cuadrado con 1 grado de libertad. La gamma es reproductiva respecto al primer parámetro, por lo que sumas de normales ($N(0,1)$) nos proporcionan gammas de parámetros $n/2$ y $1/2$, o lo que es lo mismo, chi-cuadrado con $n$ grados de libertad.

La t de Student se crea a partir de una normal ($N(0,1)$) y una chi-cuadrado con $n$ grados de libertad independientes. Una variable se distribuye bajo una t de Student si se puede definir como normal ($N(0,1)$) dividido por la raíz cuadrada de una chi-cuadrado partida por sus grados de libertad; difícil de comprender así, mejor veamos un ejemplo:

```r
# Z1, Z2, Z3, Z4 variables aleatorias independientes
# Z1 / sqrt((Z2 + Z3 + Z4) / 3) esto se distribuye según
...
# F = (X / m) / (Y / n) es F de Snedecor con m, n grados de libertad
```
...
Bueno, pues comencemos con R. La función que nos ofrece tanto estimaciones puntuales como intervalos de confianza como contrastes de hipótesis es `t.test(x, ...)`:
...
Esta es la salida que nos ofrece la ayuda de la función `t.test` (`?t.test`). Podemos poner sólo un conjunto de datos para muestras unidimensionales (estimaciones puntuales) o los dos conjuntos para comparación de muestras. El argumento `alternative` indica el tipo de contraste: bilateral `two.sided`; si la hipótesis alternativa es mayor ($H_0$: menor o igual), se utiliza `greater`; si la hipótesis alternativa es menor ($H_0$: mayor o igual), entonces se usa `less`. En `mu` indicamos el valor de la hipótesis nula.

En `paired = TRUE` estamos ante una situación de datos apareados; para indicar que estamos ante datos no apareados, poner `paired = FALSE`.

Con `var.equal` estamos trabajando con los casos de igualdad o no de varianzas, que sólo se emplean en comparación de dos poblaciones. Si `var.equal = TRUE`, las varianzas de las dos poblaciones se suponen iguales; si `var.equal = FALSE`, las varianzas de ambas poblaciones no se suponen iguales.

Por último, tenemos el argumento `conf.level` en el que indicamos el nivel de confianza del test.

Si deseáramos hacer el contraste para la igualdad de varianzas (cociente de varianzas = 1), habríamos de emplear la función `var.test(x, ...)`:
...
```r
alturas <- c(1.75, 1.82, 1.70, 1.77, 1.73, 1.85, 1.79, 1.72, 1.80, 1.76, 1.74, 1.78, 1.81, 1.73, 1.71)
t.test(alturas)
# One Sample t-test
```

El intervalo de confianza es (1,717; 1,809). No ha sido necesario modificar las opciones de la función; si contrastamos la hipótesis nula "los españoles miden 177 cm", necesitamos la opción `mu`:

```r
t.test(alturas, mu = 1.77)
```

El p-valor es la probabilidad de aceptar la hipótesis nula; en este caso es 0,7602, luego no se rechaza que la altura de los españoles es de 177 cm.
...
Admitiendo que el número de pólizas contratadas mensualmente por los dos trabajadores son variables aleatorias independientes y distribuidas normalmente: ¿Tienen igual varianza? ¿Se puede aceptar la hipótesis del director de la sucursal en función de los resultados de la tabla y a un nivel de confianza del $99\%$?
