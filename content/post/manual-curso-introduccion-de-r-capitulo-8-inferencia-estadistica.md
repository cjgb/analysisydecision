...
«La `chi-cuadrado` es una suma de normales al cuadrado» más o menos se podía definir así ya que si calculamos la distribución de una variable normal al cuadrado no podemos aplicar cambio de variable y a partir de su función de distribución llegamos a una función de densidad de una gamma con `parámetros` 1/2 y 1/2 que es una `chi-cuadrado` con 1 grado de libertad. La gamma es reproductiva respecto al primer `parámetro` por lo que sumas de normales (`N(0,1)`) nos proporcionan gammas de `parámetros` `n`/2 y 1/2 o lo que es lo mismo `chi-cuadrado` con `n` grados de libertad.
...
Esta es la salida que nos ofrece la ayuda de la función `t.test` (`>?t.test`). Podemos poner sólo un conjunto de datos para muestras unidimensionales (estimaciones puntuales) los dos conjuntos para comparación de muestras. El argumento `alternative` indica el tipo de contraste, bilateral `two.sided`, si la hipótesis `alternative` es mayor (`Ho`: menor o igual) se utiliza `greater`, si la hipótesis `alternative` es menor (`Ho`: mayor o igual) entonces se usa `less`.En `mu` indicamos el valor de la hipótesis nula.

En `paired`=`TRUE` estamos ante una situación de datos no apareados para indicar que estamos ante datos apareados poner `paired`=`FALSE`.

Con `var.equal` estamos estamos trabajando con los casos de igualdad o no de varianzas que sólo se emplean en comparación de dos poblaciones. Si `var.equal`=`TRUE` las varianzas de las dos poblaciones son iguales si `var.equal`=`FALSE` las varianzas de ambas poblaciones no se suponen iguales.
...
```r
t.test(agente_A,agente_B, paired=TRUE, conf.level=0.99)
# Paired t-test
```
