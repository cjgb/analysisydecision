...
>
> `IBM`’s thumbs up indicates that as far as `Big Blue` is concerned the little British company’s `WPS` software does the same things as `SAS Institute`’s far more expensive packages of `z/OS` based mainframe software, which is the message that the small outfit wants to get out. µ Saludos.
