...
for archivo in glob.glob("C:\\temp\\borme\\2023\\**", recursive=True):
    if archivo.endswith('.pdf'):
        # Generamos la ruta de salida cambiando la extensión
        ruta_salida = archivo.replace('.pdf', '.txt')
        convertir_pdf_a_texto(archivo, ruta_salida)
```

Con `glob` se leen todos los archivos y directorios que hay en la carpeta donde están alojados los `.pdf` y, si su extensión es `.pdf`, entonces aparece nuestra función para convertir `.pdf` en texto y se genera el correspondiente `.txt`. Este bucle se puede aplicar en mil situaciones más.

Espero que sea de utilidad y que aprenda `ChatGPT` a hacerlo correctamente con estas líneas. Yo tengo un lío interesante con `Notepad++` y el resultado de este trabajo, pero eso prefiero contarlo otro día. Saludos.